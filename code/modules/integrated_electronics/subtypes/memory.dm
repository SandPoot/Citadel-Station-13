...
/obj/item/integrated_circuit/memory/constant/attack_self(mob/user)
	...
	switch(type_to_use)
		if("string")
			...
			new_data = input("Now type in a string.", "[src] string writing", istext(O.data) ? O.data : null) as null|text
			if(istext(new_data) && user.IsAdvancedToolUser())
				O.data = new_data
				to_chat(user, span_notice("You set \the [src]'s memory to [O.display_data(O.data)]."))
		if("number")
			accepting_refs = FALSE
			new_data = input("Now type in a number.","[src] number writing", isnum(O.data) ? O.data : null) as null|num
