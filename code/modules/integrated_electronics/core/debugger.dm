...
/obj/item/integrated_electronics/debugger/attack_self(mob/user)
	...
	switch(type_to_use)
		if("string")
			...
			copy_values = FALSE
			new_data = stripped_input(user, "Now type in a string.","[src] string writing", istext(data_to_write) ? data_to_write : null, no_trim = TRUE)
		...
		if("number")
			accepting_refs = FALSE
			copy_values = FALSE
			new_data = input(user, "Now type in a number.","[src] number writing", isnum(data_to_write) ? data_to_write : null) as null|num
