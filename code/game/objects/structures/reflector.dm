...
/obj/structure/reflector/proc/rotate(mob/user)
	if (!can_rotate || admin)
		...
		return FALSE
	var/new_angle = input(user, "New angle for primary reflection face", "Reflector angle", rotation_angle) as num|null
	if(isnull(new_angle) || QDELETED(user) || QDELETED(src) || !usr.canUseTopic(src, BE_CLOSE, ismonkey(user), NO_TK))
		return FALSE
